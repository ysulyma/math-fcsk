...
export const MJX = forwardRef<Handle, Props>(function MJX(props, ref) {
  ...
  useEffect(() => {
    MathJaxReady.then(() => {
      ...
      if (!span && isSpan(spanRef.current)) {
        ...
        for (let i = 0, len = spanRef.current.classList.length; i < len; ++i) {
          element.classList.add(spanRef.current.classList.item(i) as string);
        }
...
function isSpan(element: HTMLElement): element is HTMLSpanElement {
  return element.nodeName.toLowerCase() === "span";
}
